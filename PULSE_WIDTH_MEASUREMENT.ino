 ...
 * PURPOSE : TO MEASURE THE PULSE WIDTH.
 */
...
long unsigned int t_p1 = 0;  // Variable to store a occurance timestamp of pulse_1
long unsigned int t_p2 = 0;  // Varible to store a occurance timestamp of pulse_2
...
void setup()
{
  ...
  TCCR1B = 0xC5; // Enable Noice Cancel on ICP1, RISING  EDGE Triggering, Prescalar = 1024
  TCCR1C = 0x00; // no use
  Serial.begin(9600);
  ...
  if( pulse_1_occured && pulse_2_occured )
    {
      ...
      TCCR1B = 0xC5; // set interrupt for rising edge
    }
  ...
  if( ! pulse_1_occured )
   {
    ...
    t_p1 = ICR1;
    TCCR1B = 0x85;  // set interrupt for rising edge
    no_of_cycle = 0;
   }
  else 
   {
